...
958.508 s 记录 → **4792 个整段**（958.4 s）+ 0.108 s 尾巴；尾巴不足一段不发段，记入
summary 的 `tail_s`。`--duration-sec N`（N>0）在段边界截断。
...
2. **分段器**：sub-001 全量 = 4792 段、每段 50 点、段间无重叠无空隙、尾段 0.108 s 不发段；
   `--duration-sec` 在段边界截断。
